...
export default function Recommendation({ name, youtubeLink, score, id, onUpvote = () => 0, onDownvote = () => 0 }) {
  ...
  return (
    <Container>
      <Row data-cy="Recommendation">{name}</Row>
      <ReactPlayer url={youtubeLink} width="100%" height="100%" data-cy={`YT-${name}`}/>
      <Row>
        <GoArrowUp size="24px" onClick={handleUpvote} data-cy="Like"/>
        {<div data-cy="Score">{score}</div>}
        <GoArrowDown size="24px" onClick={handleDownvote} data-cy="Dislike"/>
      </Row>
    </Container>
  );
}
